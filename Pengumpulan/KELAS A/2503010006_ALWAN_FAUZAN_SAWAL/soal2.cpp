...
void push(string judul) {
    if (isStackFull()) {
        cout << "Gagal! Tumpukan penuh untuk buku: " << judul << endl;
    } else {
        top++;
        stackBuku[top] = judul;
        cout << "Berhasil push buku: " << judul << endl;
    }
}

void pop() {
    if (isStackEmpty()) {
        cout << "Gagal! Tumpukan sudah kosong." << endl;
    } else {
        cout << "Berhasil pop buku: " << stackBuku[top] << endl;
        top--;
    }
}
...
void enqueue(string nama) {
    if (isQueueFull()) {
        cout << "Gagal! Antrian penuh untuk: " << nama << endl;
    } else {
        rear = (rear + 1) % 5;
...
void dequeue() {
    if (isQueueEmpty()) {
        ...
        cout << queueAnggota[front] << " selesai meminjam buku." << endl;
        front = (front + 1) % 5;
        count--;
    }
}
